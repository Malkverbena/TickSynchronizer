// Tests canonical varuint and ZigZag varint codecs.
// Covers boundaries, malformed forms, alignment, overflow, and truncation.
...
TEST_CASE("[Modules][TickSynchronizer][Varint] Varuint boundary values round trip") {
	...
	Ref<TickSynchronizerBuffer> script_reader;
	script_reader.instantiate();
	REQUIRE(script_reader->begin_read(writer->get_data(), writer->get_bit_size()) == OK);
	for (std::size_t index = 0; index + 1 < sizeof(values) / sizeof(values[0]); index++) {
		const Variant result = script_reader->call(SNAME("read_varuint"));
		REQUIRE(result.get_type() == Variant::INT);
		CHECK(static_cast<int64_t>(result) == static_cast<int64_t>(values[index]));
	}
	const int64_t position_before_range_error = script_reader->get_bit_position();
	const Variant out_of_range_result = script_reader->call(SNAME("read_varuint"));
	CHECK(out_of_range_result.get_type() == Variant::INT);
	CHECK(static_cast<int64_t>(out_of_range_result) == 0);
	CHECK(script_reader->get_bit_position() == position_before_range_error);
	CHECK(script_reader->get_last_error() == ERR_PARAMETER_RANGE_ERROR);
}
