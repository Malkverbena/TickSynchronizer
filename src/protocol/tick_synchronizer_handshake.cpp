...
bool ProtocolHandshakeEvaluator::is_profile_well_formed(
	...
	if (is_zero_sha1(p_profile.godot_commit) ||
...
ProtocolHandshakeEvaluation ProtocolHandshakeEvaluator::evaluate_profiles(
	...
	if (p_local.godot_commit != p_remote.godot_commit) {
		evaluation.result = ProtocolHandshakeResult::GODOT_COMMIT_MISMATCH;
		evaluation.identity_field = ProtocolIdentityField::GODOT_COMMIT;
		return evaluation;
	}
	if (p_local.module_build_id != p_remote.module_build_id) {
		evaluation.result = ProtocolHandshakeResult::MODULE_BUILD_MISMATCH;
