...
static void write_profile(uint8_t *p_destination, const ProtocolCompatibilityProfile &p_profile) {
	write_fixed_bytes(p_destination + 0, p_profile.godot_commit);
	write_fixed_bytes(p_destination + 20, p_profile.module_build_id);
	write_fixed_bytes(p_destination + 40, p_profile.game_build_id);
	write_fixed_bytes(p_destination + 56, p_profile.schema_compatibility_id);
	write_u64_le(p_destination + 72, p_profile.capabilities.supported);
	write_u64_le(p_destination + 80, p_profile.capabilities.required);
}
...
static ProtocolCompatibilityProfile read_profile(
	...
	profile.godot_commit = read_fixed_bytes<PROTOCOL_SHA1_SIZE>(p_source + 0);
	profile.module_build_id = read_fixed_bytes<PROTOCOL_SHA1_SIZE>(p_source + 20);
	profile.game_build_id = read_fixed_bytes<PROTOCOL_OPAQUE_ID_SIZE>(p_source + 40);
	profile.schema_compatibility_id = read_fixed_bytes<PROTOCOL_OPAQUE_ID_SIZE>(p_source + 56);
	profile.capabilities.supported = read_u64_le(p_source + 72);
	profile.capabilities.required = read_u64_le(p_source + 80);
	return profile;
}
...
ProtocolCodecError TickSynchronizerPacketCodec::encode_hello_ack_payload(
	...
	write_u64_le(destination + 100, p_ack.negotiated_capabilities);
	r_payload = payload;
	return ProtocolCodecError::OK;
}
...
ProtocolCodecError TickSynchronizerPacketCodec::decode_hello_ack_payload(
	...
	decoded.profile = read_profile(source + 12, precision);
	decoded.negotiated_capabilities = read_u64_le(source + 100);
	r_ack = decoded;
	return ProtocolCodecError::OK;
}
...
ProtocolCodecError TickSynchronizerPacketCodec::encode_disconnect_payload(
	...
	write_u32_le(destination + 12, p_disconnect.detail_code);
	write_u64_le(destination + 16, p_disconnect.peer_id);
	write_u64_le(destination + 24, p_disconnect.detail_mask);
	r_payload = payload;
	return ProtocolCodecError::OK;
}
...
ProtocolCodecError TickSynchronizerPacketCodec::decode_disconnect_payload(
	...
	decoded.detail_code = read_u32_le(source + 12);
	decoded.peer_id = read_u64_le(source + 16);
	decoded.detail_mask = read_u64_le(source + 24);
